...
## 4. Открыть доступ по IP

`docker-compose.yml` уже публикует порт на всех интерфейсах (`0.0.0.0:8000`),
поэтому панель доступна по `http://IP-сервера:8000`. Откройте порт в фаерволе:
...
Готово: заходите на **`http://2.26.16.197:8000`** (подставьте свой IP), вводите
общий пароль из `.env` — и работаете.

> ⚠️ Это **HTTP без шифрования**: пароль и содержимое писем идут по сети открыто.
> Вход защищён общим паролем, но канал — нет. Если данные чувствительны или
> сервер в открытом интернете надолго — закройте панель за HTTPS (шаг 5,
> необязательный). Панель всегда требует пароль (`MAILSENDER_ACCESS_PASSWORD`),
> без него не запускайте — иначе это открытый спам-релей.

---

## 5. (Необязательно) HTTPS через nginx + Let's Encrypt

Шаг для тех, кому нужен шифрованный доступ по доменному имени вместо голого IP.
Требуется домен, указывающий на сервер. После настройки закройте прямой порт:
верните в `docker-compose.yml` привязку `"127.0.0.1:8000:8000"`, выполните
`docker compose up -d`, и `sudo ufw delete allow 8000/tcp`.
...
4. `ufw`: открыты 22 (SSH) и 8000 → зашли по `http://IP:8000`, ввели пароль
5. Настроили SMTP → тестовое письмо себе дошло
6. SPF/DKIM/DMARC в DNS → проверка в UI (Настройки → Проверить домен) зелёная
7. (Необязательно) nginx + certbot → доступ по `https://домен`
